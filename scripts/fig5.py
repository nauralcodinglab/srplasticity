...
def plot_mu_sig(axes1):
    ...
    axes1[0].plot(range(1, 6), mean_om1[where(wspktr1 > 0)[0]] / munorm1, "s-k", ms=3, lw=1)  
    ...
    axes1[1].plot(range(1, 6), sig1[where(wspktr1 > 0)[0]] / munorm1, "s-g", ms=3, lw=1, color=c_green)
    axes1[1].plot(range(1, 6), sig2[where(wspktr2 > 0)[0]] / munorm2, "s-r", ms=3, lw=1, color=c_red)
    axes1[1].plot(range(1, 6), sig3[where(wspktr3 > 0)[0]] / munorm3, "s-b", ms=3, lw=1, color=c_blue)
    
    #Plot panel F; coefficient of variation from the combination of mu and sigma kernels
    axes1[2].plot(range(1, 6),sig1[where(wspktr1 > 0)[0]] / (mean_om1[where(wspktr1 > 0)[0]]),"s-g",ms=3,lw=1,color=c_green,)
    axes1[2].plot(range(1, 6),sig2[where(wspktr2 > 0)[0]] / (mean_om2[where(wspktr2 > 0)[0]]),"s-r",ms=3,lw=1,color=c_red,)
    axes1[2].plot(range(1, 6),sig3[where(wspktr3 > 0)[0]] / (mean_om3[where(wspktr3 > 0)[0]]),"s-b",ms=3,lw=1,color=c_blue,)
...
def plot_histograms(axes2):
    ...
    axes2[0].hist(x1 / munorm1, 200, range=(0, 20), density=True, color=c_green, histtype=u"step", lw=0.75)
    axes2[0].hist(x2 / munorm2, 200, range=(0, 20), density=True, color=c_red, histtype=u"step", lw=0.75)
    axes2[0].hist(x3 / munorm3, 200, range=(0, 20), density=True, color=c_blue, histtype=u"step", lw=0.75)
    
    axes2[1].hist(x4 / munorm1, 200, range=(0, 20), density=True, color=c_green, histtype=u"step", lw=0.75)
    axes2[1].hist(x5 / munorm2, 200, range=(0, 20), density=True, color=c_red, histtype=u"step", lw=0.75)
    axes2[1].hist(x6 / munorm3, 200, range=(0, 20), density=True, color=c_blue, histtype=u"step", lw=0.75)
...
if __name__ == "__main__":
    ...
    fig = plot()
    #fig.savefig(parent_dir + "/Figures/Fig5_raw.pdf")
