...
def plot_spiketrain(axZ):
    ...
    axZ.set_yticklabels([])
    axZ.plot(
        np.array([1600, 1800]), np.array([0.8, 0.8]), lw=2, c="k", solid_capstyle="butt"
    )
    axZ.axis("off")
...
def deterministic_STF(axes_STF):
    ...
    axB.set_ylim((-2.5, 9))
    axB.plot(
        np.array([1600, 1800]), np.array([4.8, 4.8]), lw=2, c="k", solid_capstyle="butt"
    )

    axC.plot(t, mean_om, "k", lw=linewidth)
    axC.plot(
        t[np.where(spktr > 0)[0]],
        mean_om[np.where(spktr > 0)[0]],
        "ok",
        ms=1,
        lw=linewidth,
        marker=None,
...
def deterministic_STDSTF(axes_STDSTF):
    ...
    axK.plot(
        t[:10000] - 200,
        np.roll(a1 * k[:10000] + a2 * k2[:10000], 2000),
        "k",
        lw=linewidth,
    )
