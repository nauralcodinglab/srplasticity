...
parameters = {
    ...
    'A': [np.nan]}  # [np.nan] because fitted to normalized data.
...
grid, estimates = do_GridSearch(ISIpatterns=ISIpatterns, parameters=parameters, synapse=synapse)
...
if Bootstrap == True:
    ...
    for sol in range(len(estimates)):
        ...
        if sol in signal:
            print('Calculated {}% of Loss function.'.format((sol / len(estimates)) * 100))
...
bestsol['mse'] = sse_eval[argbest] / np.sum([np.count_nonzero(~np.isnan(x)) for x in data.values()])

bestsol['params'] = dict(grid.iloc[argbest])
bestsol['est'] = estimates[argbest]

save_pickle(bestsol, directory / 'PN_parameters.pkl')
grid.iloc[np.argsort(sse_eval)[:50]].to_csv(directory / 'PN_best50.csv',
...
for key in protocols:
    ...
    fig.panels[panel].plot(range(1, data[key].shape[1]+1), data[key].T, color='grey', lw=0.2)
    fig.panels[panel].plot(range(1, data[key].shape[1]+1), np.nanmean(data[key], 0),
                           color='black', marker='s', label='data mean')
    fig.panels[panel].plot(range(1, data[key].shape[1]+1), bestsol['est'][key],
                           color='darkred', marker='o', label='TM model')
