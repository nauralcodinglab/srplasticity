...
class det_synapse(object):
    ...
    def __init__(self, k, b, dt=0.1, nlin="sigmoid", n=1, w=1):
...
class det_exp(det_synapse):
    ...
    def __init__(self, a, tau, b, T, dt=0.1, nlin="sigmoid", n=1, w=1):
...
class det_gaussian(det_synapse):
    ...
    def __init__(self, acommon, a, mu, sigma, b, T, dt=0.1, nlin="sigmoid", n=1, w=1):
...
def gaussian_kernel(acommon, amps, mus, sigmas, T, dt=0.1):
    ...
    for i in range(n):
        ...
        kernel += (
            a * np.exp(-((t - mu) ** 2) / 2 / sig ** 2) / np.sqrt(2 * np.pi * sig ** 2)
        )
...
if __name__ == "__main__":
    ...
    sigmas = [0.5e3, 1.2e3, 2.5e3]
    amps = [600, 4000, 8000]

    ktot = gaussian_kernel(acommon, amps, mus, sigmas, T, dt)

    k1 = gaussian_kernel(
        acommon, np.array([amps[0]]), np.array([mus[0]]), np.array([sigmas[0]]), T, dt
    )
    k2 = gaussian_kernel(
        acommon, np.array([amps[1]]), np.array([mus[1]]), np.array([sigmas[1]]), T, dt
    )
    k3 = gaussian_kernel(
        acommon, np.array([amps[2]]), np.array([mus[2]]), np.array([sigmas[2]]), T, dt
    )
