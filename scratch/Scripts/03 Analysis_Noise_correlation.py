...
for key in ['20', '100', '20100', '10020', '111', 'invivo']:
    ...
    fig.panels[panel].hlines(y=0, xmax=np.nanmax(paired[:, 0]), xmin=np.nanmin(paired[:, 0]),
                             color='darkred')
    fig.panels[panel].vlines(x=0, ymax=np.nanmax(paired[:, 1]), ymin=np.nanmin(paired[:, 1]),
                             color='darkred')
