...
axA2.plot(range(1, 6), gam[where(wspktr > 0)[0]] / (mean_om[where(wspktr > 0)[0]]), 's-g', ms=3, lw=1, color=col)
...
axF.hist(x / munorm, 200, range=(0, 20), density=True, color=col, histtype=u'step', lw=0.75 )
...
x = random.gamma(gamma, scale=mu / gamma, size=1000)
axG.hist(x / munorm, 200, range=(0, 20), density=True, color=col, histtype=u'step', lw=0.75 )
axG.set_xlim((0, 6))
...
axA2.plot(range(1, 6), gam[where(wspktr > 0)[0]] / (mean_om[where(wspktr > 0)[0]]), 's-r', ms=3, lw=1, color=col)
axA3.plot(range(1, 6), gam[where(wspktr > 0)[0]] / munorm, 's-r', ms=3, lw=1, color=col)

# axC = fig1.add_subplot(gs[6, 0])
for trial in range(Ntrial):
    axC.plot(t, I[trial, :] - 15, '.6', lw=.5, color = c_traces)
...
execfile('../flexible-STP/ProbabilisticSTP.py')
axB.plot(append(-t[::-1], t), append(k * 0 + b2, k * a2 + b2), color=col)
axA2.plot(range(1, 6), gam[where(wspktr > 0)[0]] / (mean_om[where(wspktr > 0)[0]]), 's-b', ms=3, lw=1, color=col)
axA3.plot(range(1, 6), gam[where(wspktr > 0)[0]] / munorm, 's-b', ms=3, lw=1, color=col)

for trial in range(Ntrial):
    axC.plot(t, I[trial, :] - 30, '.6', lw=.5, color = c_traces)
