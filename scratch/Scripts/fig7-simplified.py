...
def search1(J2, X):
    ...
    param0 = beta1[0, 0]
    param1 = beta2[0, 0]
    prange0 = linspace(param0 * 0.5, param0 * 1.5, 200)
    prange1 = linspace(param1 * 0.5, param1 * 1.5, 200)
    ...
    for p0 in prange0:
        for p1 in prange1:
            beta1[0, 0] = p0
            beta2[0, 0] = p1
            NLL0[where(prange0 == p0)[0], where(prange1 == p1)[0]] = neglogli(
                J2, X, beta1, beta2, sig0, mu0
            )
    cre0, cre1 = param0, param1
    cpr0, cpr1 = (
        prange0[where(NLL0 == NLL0.min())[1][0]],
        prange1[where(NLL0 == NLL0.min())[0][0]],
    )
...
def search2(J2, X):
    ...
    for p0 in prange0:
        for p1 in prange1:
            beta1[0, 1] = p0
            beta2[0, 1] = p1
            NLL1[where(prange0 == p0)[0], where(prange1 == p1)[0]] = neglogli(
                J2, X, beta1, beta2, sig0, mu0
            )
    dre0, dre1 = param0, param1
    dpr0, dpr1 = (
        prange0[where(NLL1 == NLL1.min())[1][0]],
        prange1[where(NLL1 == NLL1.min())[0][0]],
    )
...
def search3(J2, X):
    ...
    for p0 in prange0:
        for p1 in prange1:
            beta1[0, 1] = p1
            sig0 = p0
            NLL2[where(prange0 == p0)[0], where(prange1 == p1)[0]] = neglogli(
                J2, X, beta1, beta2, sig0, mu0
            )
    ere0, ere1 = param0, param1
    epr0, epr1 = (
        prange0[where(NLL2 == NLL2.min())[1][0]],
        prange1[where(NLL2 == NLL2.min())[0][0]],
    )
...
def search4(J2, X):
    ...
    for p0 in prange0:
        for p1 in prange1:
            sig0 = p0
            beta1[0, 0] = p1
            NLL3[where(prange0 == p0)[0], where(prange1 == p1)[0]] = neglogli(
                J2, X, beta1, beta2, sig0, mu0
            )
    fre0, fre1 = param0, param1
    fpr0, fpr1 = (
        prange0[where(NLL3 == NLL3.min())[1][0]],
        prange1[where(NLL3 == NLL3.min())[0][0]],
    )
...
gs = GridSpec(6, 4, wspace=0.7, bottom=0.2, hspace=0.7)
...
axD.plot(
    prange2[where(NLL1 == NLL1.min())[1][0]],
    prange3[where(NLL1 == NLL1.min())[0][0]],
    "*r",
)
...
axE.plot(
    prange4[where(NLL2 == NLL2.min())[1][0]],
    prange5[where(NLL2 == NLL2.min())[0][0]],
    "*r",
)
...
axF.plot(
    prange6[where(NLL3 == NLL3.min())[1][0]],
    prange7[where(NLL3 == NLL3.min())[0][0]],
    "*r",
)

c0err_n1, c1err_n1, d0err_n1, d1err_n1, e0err_n1, e1err_n1, num1 = param_error(
    3, 2000, dt, tauk, beta, ntrials, 3.0, baseline=False
)
c0err_n2, c1err_n2, d0err_n2, d1err_n2, e0err_n2, e1err_n2, num2 = param_error(
    3, 5000, dt, tauk, beta, ntrials, 3.0, baseline=False
)
c0err_n3, c1err_n3, d0err_n3, d1err_n3, e0err_n3, e1err_n3, num3 = param_error(
    3, 7000, dt, tauk, beta, ntrials, 3.0, baseline=False
)
Ns = array([num1, num2, num3])

c0err, c1err, d0err, d1err, e0err, e1err, num = param_error(
    3, 5000, dt, tauk, beta, ntrials, -3.0, baseline=True
)
E1 = array([mean(c0err), mean(c1err), mean(e0err)])
SE1 = array([sp.stats.sem(c0err), sp.stats.sem(c1err), sp.stats.sem(e0err)])
c0err, c1err, d0err, d1err, e0err, e1err, num = param_error(
    3, 5000, dt, tauk, beta, ntrials, 1.0, baseline=True
)
...
axG.errorbar(
    Ns,
    100 * array([mean(c0err_n1), mean(c0err_n2), mean(c0err_n3)]),
    yerr=100
    * array([sp.stats.sem(c0err_n1), sp.stats.sem(c0err_n2), sp.stats.sem(c0err_n3)]),
    fmt="o-",
    label="$a_{\sigma}$",
    color="#009988",
)
axG.errorbar(
    Ns,
    100 * array([abs(mean(d0err_n1)), abs(mean(d0err_n2)), abs(mean(d0err_n3))]),
    yerr=100
    * array(
        [
            abs(sp.stats.sem(d0err_n1)),
            abs(sp.stats.sem(d0err_n2)),
            abs(sp.stats.sem(d0err_n3)),
        ]
    ),
    fmt="o-",
    label="b$_{\sigma}$",
...
axG.errorbar(
    Ns,
    100 * array([mean(e0err_n1), mean(e0err_n2), mean(e0err_n3)]),
    yerr=100
    * array([sp.stats.sem(e0err_n1), sp.stats.sem(e0err_n2), sp.stats.sem(e0err_n3)]),
    fmt="o-",
    label="$\sigma_0$",
    color="#0077BB",
)
...
axG2.errorbar(
    Ns,
    100 * array([mean(c1err_n1), mean(c1err_n2), mean(c0err_n3)]),
    yerr=100
    * array([sp.stats.sem(c1err_n1), sp.stats.sem(c1err_n2), sp.stats.sem(c0err_n3)]),
    fmt="o-",
    label="$a_{\mu}$",
    color="#009988",
)
axG2.errorbar(
    Ns,
    100 * array([abs(mean(e1err_n1)), abs(mean(e1err_n2)), abs(mean(e1err_n3))]),
    yerr=100
    * array(
        [
            abs(sp.stats.sem(e1err_n1)),
            abs(sp.stats.sem(e1err_n2)),
            abs(sp.stats.sem(e1err_n3)),
        ]
    ),
    fmt="o-",
    label="b$_{\mu}$",
...
axH.errorbar(
    mu_baseline,
    100 * array([E1[0], E2[0], E3[0]]),
    yerr=100 * array([SE1[0], SE2[0], SE3[0]]),
    fmt="o-",
    label="$a_{\sigma}$",
    color="#009988",
)
axH.errorbar(
    mu_baseline,
    100 * array([E1[1], E2[1], E3[1]]),
    yerr=100 * array([SE1[1], SE2[1], SE3[1]]),
    fmt="o-",
    label="$a_{\mu}$",
    color="#EE7733",
)
axH.errorbar(
    mu_baseline,
    100 * array([E1[2], E2[2], E3[2]]),
    yerr=100 * array([SE1[2], SE2[2], SE3[2]]),
    fmt="o-",
    label="$\sigma_0$",
    color="#0077BB",
)
...
axI.errorbar(
    NS2,
    array([mean(ISE1), mean(ISE2), mean(ISE3)]),
    yerr=array([sp.stats.sem(ISE1), sp.stats.sem(ISE2), sp.stats.sem(ISE3)]),
    fmt="ko-",
)
