...
axA1.plot(
    range(1, 6), mean_om[where(wspktr > 0)[0]] / munorm, "s-k", ms=3, lw=1, color=c_25ca
)
...
axA3.plot(
    range(1, 6), gam[where(wspktr > 0)[0]] / munorm, "s-k", ms=3, lw=1, color=c_25ca
)
axA3.set_ylim((-0.1, 2))
axA3.set_ylabel("S.D.")
...
axA1.plot(
    range(1, 6), mean_om[where(wspktr > 0)[0]] / munorm, "s-r", ms=3, lw=1, color=c_13ca
)
axA2.plot(
    range(1, 6),
    gam[where(wspktr > 0)[0]] / (mean_om[where(wspktr > 0)[0]]),
...
axA3.plot(
    range(1, 6), gam[where(wspktr > 0)[0]] / munorm, "s-r", ms=3, lw=1, color=c_13ca
)

# axC = fig1.add_subplot(gs[6, 0])
for trial in range(Ntrial):
    axC.plot(t, I[trial, :] - 8, ".6", lw=0.5, color=c_traces)
